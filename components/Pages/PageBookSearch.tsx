...
export default async function PageBookSearch({ status, sortKey, q, page, IdCategory }: IProps) {
    const limit = 20
    const categories = await getListCategory({ limit: 20 } as IFilter);
    const { data: bookData } = await getListBooks({ categories: [IdCategory], status, sortKey, page, limit } as IFilter) || { data: [] }
    return (
        <main className="w-screen   lg:w-[1200px] dark:bg-[#252525] text-black dark:text-white bg-white flex flex-col gap-5 p-3 m-auto">
            ...
            <section className="lg:grid grid-cols-12 gap-6 ">
                <PageBookSearchFilter categories={categories} bookData={bookData} limit={limit} page={page} q={q} status={status || 0} sortKey={sortKey || ""} />
