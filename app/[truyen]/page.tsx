...
export default async function Home({ searchParams, params }: PropParams) {
  ...
  const { data: bookNew } = (await getListBooks({
    limit,
    page,
    sortKey: "views",
    status: 2,
  ...
  const { data: bookTop , total} = (await getListBooks({
    sortKey: "views",
    limit,
    page,
  } as IFilter)) || { data: [] };
  ...
  return (
    <MainLayout>
      <main className="w-full  lg:w-[1200px] pb-10 dark:bg-[#252525] dark:text-white bg-white flex flex-col gap-5 p-4 m-auto">
        <InfoCard />
        <SlideHome title={titleSlide} bookData={bookNew} />
        <div className="w-full grid grid-cols-1 gap-y-6  lg:gap-6 lg:grid-cols-12 ">
          <section className="lg:col-span-8  w-full">
            <div className="grid grid-cols-2 md:grid-cols-4  gap-4 ">
              ...
              {bookTop?.map((book: IBook, index: number) => (
